"""
Comprehensive Greek MLP Evaluation - All 6 Ciphers
...
from models.mlp_cipher_model_greek import GreekMLPDecryptor, GreekContextualMLPDecryptor, count_parameters
from models.data_loader_greek import create_dataloaders


class GreekMLPEvaluator:
    """Comprehensive evaluation for Greek MLP models"""
    ...
    def plot_confusion_matrix(self, confusion, save_path, top_k=10):
        ...
        ax.set_title(f'Top {top_k} Greek Character Confusions', fontsize=12, fontweight='bold')
        ax.invert_yaxis()
...
def load_model(model_path, vocab_size, use_context=False, device='cpu'):
    """Load trained Greek MLP model"""
    if use_context:
        model = GreekContextualMLPDecryptor(
...
def evaluate_cipher(cipher_type, use_context=False, data_dir='data/cipher_datasets_greek_1000', device='cpu'):
    """Comprehensive evaluation for one Greek cipher"""
    
    model_name = "mlp_context_greek" if use_context else "mlp_greek"
    model_display = "Greek Contextual MLP" if use_context else "Greek Simple MLP"
    ...
    if not model_path.exists():
        ...
        print(f"   Train first: python train_mlp_greek.py --cipher {cipher_type}" + 
    ...
    evaluator = GreekMLPEvaluator(model, vocab, device)
...
def evaluate_all_ciphers(use_context=False, data_dir='data/cipher_datasets_greek_1000'):
...
def compare_mlp_variants(cipher_type='caesar', data_dir='data/cipher_datasets_greek_1000'):
    """Compare Simple vs Contextual Greek MLP for a specific cipher"""
    
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    
    print(f"\n{'='*80}")
    print(f"COMPARING GREEK MLP VARIANTS: {cipher_type.upper().replace('_', ' ')}")
    print(f"{'='*80}\n")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Comprehensive Greek MLP evaluation')
    ...
    if args.compare_variants:
        if args.cipher == 'all':
            ...
            print("   Example: python evaluate_mlp_greek.py --cipher caesar --compare-variants")
        else:
