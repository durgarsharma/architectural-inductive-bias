"""
Comprehensive Greek Transformer Evaluation - All 6 Ciphers
...
from models.transformer_cipher_model_greek import GreekSimpleTransformerDecryptor, count_parameters
from models.data_loader_greek import create_dataloaders


class GreekTransformerEvaluator:
    """Comprehensive evaluation for Greek Transformer models"""
    ...
    def plot_confusion_matrix(self, confusion, save_path, top_k=10):
        ...
        ax.set_title(f'Top {top_k} Greek Character Confusions (Transformer)', fontsize=12, fontweight='bold')
        ax.invert_yaxis()
...
def load_model(model_path, vocab_size, device='cpu'):
    """Load trained Greek Transformer model"""
    model = GreekSimpleTransformerDecryptor(
...
def evaluate_cipher(cipher_type, data_dir='data/cipher_datasets_greek_1000', device='cpu'):
    ...
    print(f"\n{'='*80}")
    print(f"EVALUATING GREEK TRANSFORMER: {cipher_type.upper().replace('_', ' ')}")
    print(f"{'='*80}\n")
    
    # Load training metrics
    metrics_path = Path('results') / f'transformer_greek_{cipher_type}' / 'metrics.json'
    training_info = {}
    ...
    model_path = Path('results') / f'transformer_greek_{cipher_type}' / 'best_model.pt'
    
    if not model_path.exists():
        print(f"❌ Model not found: {model_path}")
        print(f"   Train first: python train_transformer_greek.py --cipher {cipher_type}")
        return None
    ...
    evaluator = GreekTransformerEvaluator(model, vocab, device)
    results = evaluator.evaluate_dataset(test_loader)
    
    # Print summary
    print(f"\n{'='*80}")
    print(f"COMPREHENSIVE RESULTS - GREEK TRANSFORMER - {cipher_type.upper().replace('_', ' ')}")
    print(f"{'='*80}")
    ...
    if training_info:
        ...
        print(f"\n🏋️  Training Info:")
        print(f"  Model Type:          Greek Transformer (Encoder-Only)")
    ...
    # Save results
    results_dir = Path('results') / f'transformer_greek_{cipher_type}'
...
def evaluate_all_ciphers(data_dir='data/cipher_datasets_greek_1000'):
    ...
    # Comparison table
    print("\n" + "="*100)
    print("COMPREHENSIVE COMPARISON - ALL GREEK TRANSFORMERS")
    print("="*100)
    ...
    with open(results_dir / 'all_transformer_greek_comparison.json', 'w', encoding='utf-8') as f:
    ...
    ax.set_title('Greek Transformer Performance Across All Ciphers', fontsize=12, fontweight='bold')
    ax.set_xticks(x)
    ...
    plt.tight_layout()
    plt.savefig(results_dir / 'all_transformer_greek_comparison.png', dpi=150, bbox_inches='tight')
    plt.close()
    
    print(f"\n✓ Saved comparison to results/all_transformer_greek_comparison.png")


if __name__ == "__main__":
    import argparse
    
    parser = argparse.ArgumentParser(description='Comprehensive Greek Transformer evaluation')
