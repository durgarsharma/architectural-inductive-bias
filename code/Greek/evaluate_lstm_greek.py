"""
Comprehensive Greek LSTM Evaluation - All 6 Ciphers
...
from models.lstm_cipher_model_greek import GreekSimpleLSTMDecryptor, count_parameters
from models.data_loader_greek import create_dataloaders


class GreekLSTMEvaluator:
    """Comprehensive evaluation for Greek LSTM models"""
    ...
    def plot_confusion_matrix(self, confusion, save_path, top_k=10):
        """Plot top-k most confused Greek character pairs"""
        confusions_list = []
        ...
        ax.set_title(f'Top {top_k} Greek Character Confusions (LSTM)', fontsize=12, fontweight='bold')
        ax.invert_yaxis()
...
def load_model(model_path, vocab_size, device='cpu'):
    """Load trained Greek LSTM model"""
    model = GreekSimpleLSTMDecryptor(
...
def evaluate_cipher(cipher_type, data_dir='data/cipher_datasets_greek_1000', device='cpu'):
    """Comprehensive evaluation for one Greek cipher"""
    
    print(f"\n{'='*80}")
    print(f"EVALUATING GREEK LSTM: {cipher_type.upper().replace('_', ' ')}")
    print(f"{'='*80}\n")
    
    # Load training metrics
    metrics_path = Path('results') / f'lstm_greek_{cipher_type}' / 'metrics.json'
    training_info = {}
    ...
    # Load model
    model_path = Path('results') / f'lstm_greek_{cipher_type}' / 'best_model.pt'
    
    if not model_path.exists():
        print(f"❌ Model not found: {model_path}")
        print(f"   Train first: python train_lstm_greek.py --cipher {cipher_type}")
        return None
    ...
    # Evaluate
    evaluator = GreekLSTMEvaluator(model, vocab, device)
    results = evaluator.evaluate_dataset(test_loader)
    
    # Print summary
    print(f"\n{'='*80}")
    print(f"COMPREHENSIVE RESULTS - GREEK LSTM - {cipher_type.upper().replace('_', ' ')}")
    print(f"{'='*80}")
    ...
    if training_info:
        ...
        print(f"\n🏋️  Training Info:")
        print(f"  Model Type:          Greek LSTM (Bidirectional)")
    ...
    results_dir = Path('results') / f'lstm_greek_{cipher_type}'
    ...
    print(f"  - detailed_results.json (per-article breakdown)")
    print(f"  - test_results.json (summary)")
    print(f"  - confusion_matrix.png")
    print(f"  - accuracy_distributions.png")
    
    return results


def evaluate_all_ciphers(data_dir='data/cipher_datasets_greek_1000'):
    """Evaluate all 6 Greek ciphers and create comparison"""
    ...
    print("\n" + "="*100)
    print("COMPREHENSIVE COMPARISON - ALL GREEK LSTM CIPHERS")
    print("="*100)
    ...
    with open(results_dir / 'all_lstm_greek_comparison.json', 'w', encoding='utf-8') as f:
    ...
    ax.set_title('Greek LSTM Performance Across All Ciphers', fontsize=12, fontweight='bold')
    ax.set_xticks(x)
    ...
    plt.tight_layout()
    plt.savefig(results_dir / 'all_lstm_greek_comparison.png', dpi=150, bbox_inches='tight')
    plt.close()
    
    print(f"\n✓ Saved comparison to results/all_lstm_greek_comparison.png")


if __name__ == "__main__":
    import argparse
    
    parser = argparse.ArgumentParser(description='Comprehensive Greek LSTM evaluation')
