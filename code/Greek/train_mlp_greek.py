"""
Training Script for Greek MLP - All 6 Ciphers
Simple feedforward baseline with early stopping
"""
...
class GreekMLPTrainer:
    """Trainer for Greek MLP cipher decryption"""
    ...
    def train(self, train_loader, val_loader, num_epochs=20, save_dir='results',
        ...
        print("\n" + "="*70)
        print("Starting Greek MLP Training")
        print("Εκκίνηση Εκπαίδευσης MLP για Ελληνικά")
        print("="*70)
    ...
    def plot_training_history(self, save_path='results/training_history.png', max_epochs=20):
        ...
        ax1.set_title('Greek MLP: Training and Validation Loss', fontsize=12, fontweight='bold')
        ax1.legend(loc='best')
        ...
        ax2.set_title('Greek MLP: Validation Accuracy', fontsize=12, fontweight='bold')
        ax2.legend(loc='best')
...
def main(cipher_type='caesar', num_epochs=20, data_dir='data/cipher_datasets_greek_1000',
         train_fraction=1.0, learning_rate=0.001, use_context=False,
         patience=5, min_delta=0.001):
    ...
    if train_fraction < 1.0:
        ...
        from dataloader_greek import collate_batch
    ...
    if use_context:
        ...
        model_name = "mlp_greek"
    
    print(f"Model type: {model_name}")
    print(f"Model parameters: {count_parameters(model):,}")
    print(f"Vocabulary size: {vocab.vocab_size} (Greek alphabet: 24 letters)")
    
    trainer = GreekMLPTrainer(model, vocab, device=device, learning_rate=learning_rate)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Train Greek MLP for cipher decryption')
    ...
    parser.add_argument('--use-context', action='store_true',
                       help='Use contextual MLP (with neighbor chars)')
    ...
    model_type = "Contextual MLP" if args.use_context else "Simple MLP"
    print(f"\n{'='*70}")
    print(f"Training Greek {model_type} on {args.cipher.upper().replace('_', ' ')} cipher")
