...
class CNNEvaluator:
    """Comprehensive evaluation for CNN models"""
...
def evaluate_cipher(cipher_type, use_deep=False, data_dir='data/cipher_datasets_1000', device='cpu'):
    """Comprehensive evaluation for one cipher"""
    
    model_name = "cnn_deep" if use_deep else "cnn"
    model_display = "Deep Residual CNN" if use_deep else "Multi-Kernel CNN"
    ...
    if not model_path.exists():
        ...
        print(f"   Train first: python train_cnn.py --cipher {cipher_type}" + 
    ...
    evaluator = CNNEvaluator(model, vocab, device)
...
def evaluate_all_ciphers(use_deep=False, data_dir='data/cipher_datasets_1000'):
...
def compare_cnn_variants(cipher_type='caesar', data_dir='data/cipher_datasets_1000'):
    """Compare Multi-Kernel CNN vs Deep Residual CNN for a specific cipher"""
    
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    
    print(f"\n{'='*80}")
    print(f"COMPARING CNN VARIANTS: {cipher_type.upper().replace('_', ' ')}")
    print(f"{'='*80}\n")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Comprehensive CNN evaluation')
    ...
    if args.compare_variants:
        if args.cipher == 'all':
            ...
            print("   Example: python evaluate_cnn_comprehensive.py --cipher caesar --compare-variants")
        else:
