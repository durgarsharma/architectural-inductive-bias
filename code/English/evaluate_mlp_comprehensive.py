...
class MLPEvaluator:
    """Comprehensive evaluation for MLP models"""
...
def evaluate_cipher(cipher_type, use_context=False, data_dir='data/cipher_datasets_1000', device='cpu'):
    """Comprehensive evaluation for one cipher"""
    
    model_name = "mlp_context" if use_context else "mlp"
    model_display = "Contextual MLP" if use_context else "Simple MLP"
    ...
    if not model_path.exists():
        ...
        print(f"   Train first: python train_mlp.py --cipher {cipher_type}" + 
    ...
    evaluator = MLPEvaluator(model, vocab, device)
...
def evaluate_all_ciphers(use_context=False, data_dir='data/cipher_datasets_1000'):
    ...
    with open(results_dir / f'all_{model_name}_comparison.json', 'w') as f:
    ...
    ax.set_title(f'{model_display} Performance Across All Ciphers')
    ax.set_xticks(x)
...
def compare_mlp_variants(cipher_type='caesar', data_dir='data/cipher_datasets_1000'):
    """Compare Simple MLP vs Contextual MLP for a specific cipher"""
    
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    
    print(f"\n{'='*80}")
    print(f"COMPARING MLP VARIANTS: {cipher_type.upper().replace('_', ' ')}")
    print(f"{'='*80}\n")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Comprehensive MLP evaluation')
    ...
    if args.compare_variants:
        if args.cipher == 'all':
            ...
            print("   Example: python evaluate_mlp_comprehensive.py --cipher caesar --compare-variants")
        else:
