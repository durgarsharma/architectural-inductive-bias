"""
Comprehensive Model Evaluation - All 6 Ciphers
...
def evaluate_cipher(cipher_type, data_dir='data/cipher_datasets_1000', device='cpu'):
    """Comprehensive evaluation for one cipher"""
    
    print(f"\n{'='*80}")
    print(f"EVALUATING: {cipher_type.upper().replace('_', ' ')}")
    print(f"{'='*80}\n")
    
    # Load training metrics to show training info
    metrics_path = Path('results') / f'lstm_{cipher_type}' / 'metrics.json'
    training_info = {}
    ...
    if not model_path.exists():
        ...
        print(f"   Train first: python train_lstm_all_ciphers.py --cipher {cipher_type}")
        return None
    ...
    # Evaluate
    evaluator = ComprehensiveEvaluator(model, vocab, device)
    results = evaluator.evaluate_dataset(test_loader)
    
    # Print summary
    print(f"\n{'='*80}")
    print(f"COMPREHENSIVE RESULTS - {cipher_type.upper().replace('_', ' ')}")
    print(f"{'='*80}")
...
def evaluate_all_ciphers(data_dir='data/cipher_datasets_1000'):
    ...
    print("\n" + "="*100)
    print("COMPREHENSIVE COMPARISON - ALL CIPHERS")
    print("="*100)
    ...
    # Save comparison
    with open('results/all_ciphers_comparison.json', 'w') as f:
    ...
    ax.set_title('LSTM Performance Across All Ciphers')
    ax.set_xticks(x)
    ...
    plt.tight_layout()
    plt.savefig('results/all_ciphers_comparison.png', dpi=150, bbox_inches='tight')
    plt.close()
    
    print(f"\n✓ Saved comparison to results/all_ciphers_comparison.png")


if __name__ == "__main__":
    import argparse
    
    parser = argparse.ArgumentParser(description='Comprehensive evaluation of cipher models')
