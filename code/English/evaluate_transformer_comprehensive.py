"""
Comprehensive Transformer Evaluation - All 6 Ciphers
...
class TransformerEvaluator:
    """Comprehensive evaluation for Transformer models"""
...
def evaluate_cipher(cipher_type, data_dir='data/cipher_datasets_1000', device='cpu'):
    ...
    print(f"\n{'='*80}")
    print(f"EVALUATING TRANSFORMER: {cipher_type.upper().replace('_', ' ')}")
    print(f"{'='*80}\n")
    
    # Load training metrics
    metrics_path = Path('results') / f'transformer_{cipher_type}' / 'metrics.json'
    training_info = {}
    ...
    if not model_path.exists():
        ...
        print(f"   Train first: python train_transformer_all_ciphers.py --cipher {cipher_type}")
        return None
    
    print(f"Loading model from {model_path}...")
    model = load_model(model_path, vocab.vocab_size, device)
    
    evaluator = TransformerEvaluator(model, vocab, device)
    results = evaluator.evaluate_dataset(test_loader)
    
    # Print summary
    print(f"\n{'='*80}")
    print(f"COMPREHENSIVE RESULTS - TRANSFORMER - {cipher_type.upper().replace('_', ' ')}")
    print(f"{'='*80}")
...
def evaluate_all_ciphers(data_dir='data/cipher_datasets_1000'):
    ...
    # Comparison table
    print("\n" + "="*100)
    print("COMPREHENSIVE COMPARISON - ALL TRANSFORMERS")
    print("="*100)
    ...
    # Save comparison
    with open('results/all_transformers_comparison.json', 'w') as f:
    ...
    ax.set_title('Transformer Performance Across All Ciphers')
    ax.set_xticks(x)
    ...
    plt.tight_layout()
    plt.savefig('results/all_transformers_comparison.png', dpi=150, bbox_inches='tight')
    plt.close()
    
    print(f"\n✓ Saved comparison to results/all_transformers_comparison.png")
