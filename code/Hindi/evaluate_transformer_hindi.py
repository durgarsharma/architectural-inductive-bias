"""
Comprehensive Transformer Evaluation - All 6 Ciphers (Hindi/Devanagari)
Includes: Character accuracy, Word accuracy, Edit distance, Per-article breakdown,
Full Confusion matrix with index-based plotting
"""
...
class TransformerEvaluatorHindi:
    """Comprehensive evaluation for Transformer models (Hindi)"""
    ...
    def plot_confusion_matrix_heatmap(self, confusion_matrix, save_path, 
        ...
        title = f'Confusion Matrix (Top {top_n} Character Indices) - Hindi Transformer'
        if normalize:
            title += ' - Normalized'
...
def plot_training_curves(training_info, save_path):
    ...
    summary_text = f"""
    Training Summary (Hindi Transformer)
    ─────────────────────────────
    ...
    plt.suptitle('Training Curves and Analysis (Hindi Transformer)', fontsize=14, fontweight='bold', y=0.995)
    plt.tight_layout()
    plt.savefig(save_path, dpi=150, bbox_inches='tight')
    plt.close()
...
def plot_loss_comparison(train_loss, val_loss, test_loss, save_path):
    ...
    ax.set_title('Loss Comparison Across Splits (Hindi Transformer)', fontsize=14, fontweight='bold')
...
def load_model(model_path, vocab_size, device='cp'):
...
def evaluate_cipher(cipher_type, data_dir='data/cipher_datasets_hindi_1000', device='cpu'):
    ...
    print(f"\n{'='*80}")
    print(f"EVALUATING HINDI TRANSFORMER: {cipher_type.upper().replace('_', ' ')}")
    print(f"{'='*80}\n")
    
    # Load training metrics
    metrics_path = Path('results') / f'transformer_hindi_{cipher_type}' / 'metrics.json'
    training_info = {}
    ...
    model_path = Path('results') / f'transformer_hindi_{cipher_type}' / 'best_model.pt'
    
    if not model_path.exists():
        print(f"❌ Model not found: {model_path}")
        print(f"   Train first: python train_transformer_hindi.py --cipher {cipher_type}")
        return None
    ...
    evaluator = TransformerEvaluatorHindi(model, vocab, device)
    
    # Save complete character mapping
    results_dir = Path('results') / f'transformer_hindi_{cipher_type}'
    ...
    # Print summary
    print(f"\n{'='*80}")
    print(f"COMPREHENSIVE RESULTS - HINDI TRANSFORMER - {cipher_type.upper().replace('_', ' ')}")
    print(f"{'='*80}")
    ...
    if training_info:
        ...
        print(f"\n🏋️  Training Info:")
        print(f"  Model Type:          Hindi Transformer")
    ...
    print("    • character_index_mapping.json")
    print("\n  Mapping Files:")
    print("    • character_index_mapping.txt")
    print("    • top_confusions_mapping.txt")
    print("    • confusion_matrix_index_mapping.txt")
    print("    • top_25_character_mapping.txt")
    print("\n  Visualization Files:")
    ...
    print("    • per_character_accuracy.png")
...
def evaluate_all_ciphers(data_dir='data/cipher_datasets_hindi_1000'):
    ...
    # Comparison table
    print("\n" + "="*100)
    print("COMPREHENSIVE COMPARISON - ALL HINDI TRANSFORMERS")
    print("="*100)
    ...
    with open(results_dir / 'all_hindi_transformers_comparison.json', 'w', encoding='utf-8') as f:
    ...
    axes[0, 0].set_title('Hindi Transformer - Character vs Word Accuracy', fontsize=12, fontweight='bold')
    ...
    plt.suptitle('Hindi Transformer Performance Across All Ciphers', 
                 fontsize=14, fontweight='bold', y=0.995)
    plt.tight_layout()
    plt.savefig(results_dir / 'all_hindi_transformers_comparison.png', dpi=150, bbox_inches='tight')
    plt.close()
    
    print(f"\n✓ Saved comprehensive comparison to results/all_hindi_transformers_comparison.png")
    print(f"✓ Saved comparison data to results/all_hindi_transformers_comparison.json\n")


if __name__ == "__main__":
    import argparse
    
    parser = argparse.ArgumentParser(description='Comprehensive Hindi Transformer evaluation')
