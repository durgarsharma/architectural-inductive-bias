"""
Training Script for Transformer - All 6 Ciphers (Hindi/Devanagari)
"""
...
class TransformerTrainerHindi:
    """Trainer for Transformer cipher decryption (Hindi)"""
    ...
    def train(self, train_loader, val_loader, num_epochs=20, save_dir='results', 
        ...
        print("\n" + "="*60)
        print("Starting Hindi Transformer Training")
        print("="*60)
    ...
    def plot_training_history(self, save_path='results/training_history.png'):
        ...
        ax1.set_ylabel('Loss')
        ax1.set_title('Training and Validation Loss (Hindi Transformer)')
        ax1.legend()
        ax1.grid(True)
        ...
        ax2.set_title('Validation Accuracy (Hindi Transformer)')
        ax2.legend()
        ax2.grid(True)
...
def main(cipher_type='caesar', num_epochs=20, data_dir='data/cipher_datasets_hindi_1000',
    ...
    print("\nInitializing Hindi Transformer model...")
    ...
    trainer = TransformerTrainerHindi(model, vocab, device=device, learning_rate=learning_rate)
    
    results_dir = Path('results') / f'transformer_hindi_{cipher_type}'
    results_dir.mkdir(parents=True, exist_ok=True)
    
    best_acc, epochs_trained = trainer.train(
        train_loader,
        val_loader,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Train Transformer for Hindi cipher decryption')
    ...
    print(f"\n{'='*60}")
    print(f"Training Hindi Transformer on {args.cipher.upper().replace('_', ' ')} cipher")
    ...
    print("\n✅ Hindi Transformer training complete!")