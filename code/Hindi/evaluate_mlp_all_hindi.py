"""
Comprehensive MLP Evaluation - All 6 Ciphers (Hindi/Devanagari)
...
from models.mlp_cipher_model import MLPDecryptor, ContextualMLPDecryptor, count_parameters
...
class MLPEvaluatorHindi:
    """Comprehensive evaluation for MLP models (Hindi)"""
    ...
    def plot_confusion_matrix_heatmap(self, confusion_matrix, save_path, 
        ...
        with open(mapping_path, 'w', encoding='utf-8') as f:
            f.write("="*60 + "\n")
            f.write(f"CONFUSION MATRIX - INDEX TO CHARACTER MAPPING")
...
def plot_training_curves(training_info, save_path):
    ...
    plt.suptitle('Training Curves and Analysis (Hindi MLP)', fontsize=14, fontweight='bold', y=0.995)
    plt.tight_layout()
    plt.savefig(save_path, dpi=150, bbox_inches='tight')
    plt.close()
...
def plot_loss_comparison(train_loss, val_loss, test_loss, save_path):
    ...
    ax.set_title('Loss Comparison Across Splits (Hindi MLP)', fontsize=14, fontweight='bold')
...
def load_model(model_path, vocab_size, use_context=False, device='cpu'):
    """Load trained MLP model"""
    if use_context:
        model = ContextualMLPDecryptor(
...
def evaluate_cipher(cipher_type, use_context=False, data_dir='data/cipher_datasets_hindi_1000', device='cpu'):
    """Comprehensive evaluation for one cipher"""
    
    model_name = "mlp_context_hindi" if use_context else "mlp_hindi"
    model_display = "Contextual Hindi MLP" if use_context else "Simple Hindi MLP"
    ...
    if not model_path.exists():
        ...
        print(f"   Train first: python train_mlp_hindi.py --cipher {cipher_type}" + 
              (" --use-context" if use_context else ""))
        return None
    
    print(f"Loading model from {model_path}...")
    model = load_model(model_path, vocab.vocab_size, use_context, device)
    print(f"Model parameters: {count_parameters(model):,}")
    
    evaluator = MLPEvaluatorHindi(model, vocab, device)
...
def evaluate_all_ciphers(use_context=False, data_dir='data/cipher_datasets_hindi_1000'):
    ...
    for cipher in ciphers:
        result = evaluate_cipher(cipher, use_context, data_dir, device)
        if result:
...
def compare_mlp_variants(cipher_type='caesar', data_dir='data/cipher_datasets_hindi_1000'):
    """Compare Simple MLP vs Contextual MLP for a specific cipher"""
    
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    
    print(f"\n{'='*80}")
    print(f"COMPARING HINDI MLP VARIANTS: {cipher_type.upper().replace('_', ' ')}")
    print(f"{'='*80}\n")
    
    results_simple = evaluate_cipher(cipher_type, use_context=False, data_dir=data_dir, device=device)
    results_context = evaluate_cipher(cipher_type, use_context=True, data_dir=data_dir, device=device)
    
    if not results_simple or not results_context:
    ...
    print("="*80)
    print(f"\n{'Metric':<30} {'Simple MLP':<20} {'Contextual MLP':<20} {'Difference':<15}")
    print("-"*80)
    ...
    for metric_name, metric_key in metrics:
        simple_val = results_simple['summary'][metric_key] * 100
        context_val = results_context['summary'][metric_key] * 100
        diff = context_val - simple_val
        diff_str = f"+{diff:.2f}%" if diff > 0 else f"{diff:.2f}%"
        
        print(f"{metric_name:<30} {simple_val:>8.2f}%       {context_val:>8.2f}%       {diff_str:>12}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Comprehensive Hindi MLP evaluation')
    ...
    parser.add_argument('--use-context', action='store_true',
                       help='Evaluate contextual MLP instead of simple MLP')
    parser.add_argument('--compare-variants', action='store_true',
                       help='Compare simple vs contextual MLP (requires --cipher to be specific)')
    ...
    if args.compare_variants:
        if args.cipher == 'all':
            ...
            print("   Example: python evaluate_mlp_hindi.py --cipher caesar --compare-variants")
        else:
            compare_mlp_variants(args.cipher, args.data_dir)
    elif args.cipher == 'all':
        evaluate_all_ciphers(args.use_context, args.data_dir)
    else:
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        evaluate_cipher(args.cipher, args.use_context, args.data_dir, device)